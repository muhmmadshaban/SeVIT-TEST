...
def get_classifiers_list(MLP_path):
    classifiers_list = []

    for classif in sorted(os.listdir(MLP_path)):
        full_path = os.path.join(MLP_path, classif)

        # --- IMPORTANT ---
        # Load on CPU only, DO NOT move to GPU here
        mlp = torch.load(full_path, map_location="cpu").eval()

        print(f"{classif} loaded on CPU.")
