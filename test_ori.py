...
def test_mlps(mlps_list, dataloader_test, mlp_root_dir):
    for mlp_idx in range(1, len(mlps_list) + 1):
        acc_avg = 0.0

        mlp_in = torch.load(
            os.path.join(mlp_root_dir, mlps_list[mlp_idx - 1])
        ).cuda()

        mlp_in.eval()
        print(f'MLP of index {mlp_idx - 1} has been loaded')
        ...
        print(f'Accuracy of block {mlp_idx - 1} = {(acc_avg/len(dataloader_test)):.3f}')
    pass
...
if args.model_name == 'ViT':
    ...
    acc = test_mlps(
        mlps_list=mlps_list,
        dataloader_test=loader_['test'],
        mlp_root_dir=args.mlp_path
    )
